...
class interfaceOntology:
    ...
    def __init__(self, path, sortie_rdf, sortie_owl, mode):
        self.path = path
    ...
    def create_reviewer(self, reviewerID):
        reviewer_r = rdflib.URIRef(self.rdf_ns +self.path.split(".")[0].split("\\")[-1].split("/")[-1] + "/" + reviewerID)
...
ontologie = interfaceOntology("Video_Games.json.gz", "data.rdf", "knowledge.owl", "user")
